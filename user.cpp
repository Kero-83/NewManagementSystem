...
void User::RateNew(NewsModel news, double rate) {
    int id = Login::count;
    if(news.ratedUsersIds.count(id))
    {
        news.avgRate += (rate - news.avgRate)*news.ratedUsersIds.size();
        news.rates[id] = rate;
    }
    else
    {
        news.ratedUsersIds.insert(id);
        news.avgRate = ((news.avgRate * (news.ratedUsersIds.size() - 1)) + rate) / news.ratedUsersIds.size();
        news.rates[id] = rate;
    }
}
void User::Bookmark() {}
